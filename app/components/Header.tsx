      ...
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-6">
        ...
        <div className="flex items-center gap-4">
          <nav className="flex items-center gap-6 lg:hidden">
            {navLinks.slice(0, 2).map((link) => (
              <Link
                key={link.label}
                href={link.href}
                className={`text-sm ${link.active ? "font-semibold text-navy" : "text-muted"}`}
              >
                {link.label}
              </Link>
            ))}
          ...
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-md text-muted hover:bg-slate-100 hover:text-navy"
          >
